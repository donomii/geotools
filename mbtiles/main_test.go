...
func TestExtractMBTilesRejectsShortOutputWrite(t *testing.T) {
	database, err := sql.Open("sqlite", ":memory:")
	if err != nil {
		t.Fatal(err)
	}
	defer database.Close()
	if _, err := database.Exec(`CREATE TABLE tiles (zoom_level INTEGER, tile_column INTEGER, tile_row INTEGER, tile_data BLOB); INSERT INTO tiles VALUES (0, 0, 0, x'001122')`); err != nil {
		t.Fatal(err)
	}
	err = extractMBTile(database, shortTileWriter{}, 0, 0, 0)
	if !errors.Is(err, io.ErrShortWrite) {
		t.Fatalf("short MBTiles extraction write produced error %v", err)
	}
}

type shortTileWriter struct{}

func (shortTileWriter) Write(data []byte) (int, error) {
